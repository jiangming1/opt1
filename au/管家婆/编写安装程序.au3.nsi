...
; 安装完成页面
!define MUI_FINISHPAGE_RUN "$INSTDIR\My application.exe"
...
OutFile "Output\My application.exe"
...
Section "完整" SEC01
  ...
  File "管店婆(免费)\My.exe"
  Rename $INSTDIR\my.exe $INSTDIR\My application.exe
  CreateDirectory "$SMPROGRAMS\My application"
  CreateShortCut "$SMPROGRAMS\My application\My application.lnk" "$INSTDIR\My application.exe"
  CreateShortCut "$DESKTOP\My application.lnk" "$INSTDIR\My application.exe"
