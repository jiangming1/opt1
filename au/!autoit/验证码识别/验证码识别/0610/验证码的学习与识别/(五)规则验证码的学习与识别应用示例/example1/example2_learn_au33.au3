...
;步骤2: 学习所有出现的字符
$CodeNum=5  ;5个数组成一个验证码
$CodeWidth=7 ;验证码宽度为7个点
$func_left=2 ;左边有2个点为空,不去边框的话,值+1
$func_mid=3 ;中间有3个点为空
$func_top=3 ;上方有3行为空,不去边框的话,值+1
$func_bottom=3 ;下方有3行为空,不去边框的话,值+1

$s_FontFile = @ScriptDir&"\CodeFont.txt"
FileDelete($s_FontFile)

$a_Image = myReadImageToArray(@ScriptDir&"\code\1.bmp", True, 1, 0x777777);学习82153五个数, 还缺46790没学过
$a_Image = myArrayDelBox($a_Image, 1, 1, 1, 1);去边框(边框厚度上下左右各1个点);其实不去也可以,上下左这3个参数各加1即可,只是为了方便说明这个函数的功能
myArrarOCR($s_FontFile, $a_Image, 0, $CodeNum, $CodeWidth, "82153", $func_left, $func_mid, $func_top, $func_bottom)

$a_Image = myReadImageToArray(@ScriptDir&"\code\3.bmp", True, 1, 0x777777);学习71654, 还缺90没学过
$a_Image = myArrayDelBox($a_Image, 1, 1, 1, 1)
myArrarOCR($s_FontFile, $a_Image, 0, $CodeNum, $CodeWidth, "71654", $func_left, $func_mid, $func_top, $func_bottom)

$a_Image = myReadImageToArray(@ScriptDir&"\code\22.bmp", True, 1, 0x777777);学习59027,至此0-9都学过了
$a_Image = myArrayDelBox($a_Image, 1, 1, 1, 1)
myArrarOCR($s_FontFile, $a_Image, 0, $CodeNum, $CodeWidth, "59027", $func_left, $func_mid, $func_top, $func_bottom)
