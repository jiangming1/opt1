...
$a_Image = myReadImageToArray(@ScriptDir&"\code\1.jpg", False, 1, $func_sc);学习
myArrarOCR($s_FontFile, $a_Image, 0, $CodeNum, $CodeWidth, "BYeg7J", $func_left, $func_mid, $func_top, $func_bottom)

$a_Image = myReadImageToArray(@ScriptDir&"\code\2.jpg", False, 1, $func_sc);学习
myArrarOCR($s_FontFile, $a_Image, 0, $CodeNum, $CodeWidth, "RmNy25", $func_left, $func_mid, $func_top, $func_bottom)


;步骤3: 识别
$a_Image = myReadImageToArray(@ScriptDir&"\code\3.jpg", True, 1, $func_sc);用true,false都无所谓了, 这里表示一维二维数组都自动支持
$s_code_string = myArrarOCR(@ScriptDir&"\CodeFont.txt", $a_Image, 1, $CodeNum, $CodeWidth, "", $func_left, $func_mid, $func_top, $func_bottom)
MsgBox(0, "识别结果", $s_code_string);实际为y3N9hg, 但因为上面只学习过N(大写),g和y(小写),所以只有二个字符是对的
